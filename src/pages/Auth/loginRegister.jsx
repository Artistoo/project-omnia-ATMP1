...
import { TbArrowsRandom, TbError404 } from "react-icons/tb";
...
import { fill, update } from "lodash";
  ...
  const RegisterForm = () => {
      ...
      inputs: [
        ...
        },
        {
          ...
          ready: {
            ...
            go: false,
            error: false,
            errorMSG: `please make sure the password is at least 6 letters long`,
          },
          ...
          update.map((inp) => {
            if (!inp.ready.go) {
              if (inp.ref && inp.ref.current) {
                ...
                inp.ref.current.style.opacity = 0.5;

                const InputNotValidTimerPromise = new Promise((resolve) => {
                  const InputNotValidTimer = setTimeout(() => {
                    inp.ref.current.style.border = "solid white thin";
                    inp.ref.current.style.opacity = 1;
                    done = true;
                  }, [1500]);
                  resolve();
    ...
    const handleBlur = (inputs, e) => {
      if (inputs.ready.error) {
        ...
        setForminputs((current) => ({
          ...current,
          Error: formInputs.inputs[index].ready.errorMSG,
        }));
      } else {
      ...
      if (formInputs.inputs.every((x) => x.ready.error === false)) {
        setForminputs((current) => ({ ...current, Error: "" }));
      }
    ...
    const handleChange = (index, e) => {
      ...
      setForminputs((current) => {
        ...
        if (formInputs.inputs[index].ready.error) {
          e.target.style.border = "solid red thin";
          current.Error = formInputs.inputs[index].ready.errorMSG;
        } else {
          e.target.style.border = "solid green thin";
          focused
            ? (e.target.style.border = "solid green thin")
        ...
        >
          {formInputs.Error && (
            <>
              ...
              <p className="h-max w-full pl-[75px]  pr-[20px]">
                {formInputs.Error}{" "}
              </p>
            </>
          )}
            ...
            >
              ...
              {formInputs?.inputs.map((inputs, index) => (
                ...
                ${inputs?.span ? `col-span-1` : `col-span-4`} ${
                    inputs.display.includes(formInputs.Req_Type)
                      ? ""
                      : inputs.leave
                  } ${
